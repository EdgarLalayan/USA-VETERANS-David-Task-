...
class PDFParser:
    ...
    def process_pdf_files(self):
        current_dir = '/Users/edgarlalayan/Desktop/CASPIO/Veteran /parser1.5/RatingDecisionExtractProject/SampleRatingDecisions'
        ...
        for row in data:
            ...
            if match_name:
                ...
                if '(' in decisionName:
                    ...
                    decisionName = decisionName.split('[')[0]
                decisionName = decisionName.rstrip(',').strip()
            ...
            #outcome
            #EEDD 
            if ('Entitlement' in row or 'assigned' in row) and 'denied' in row:
                outcomes.append("EEDDenied")        
            elif 'Entitlement' in row or 'assigned' in row:
                outcomes.append("EEDGranted") 
            else: 
                if 'denied' in row or 'denial' in row:
                ...
                if 'decreased' in row:
                    outcomes.append("Decreased") 
                if 'continued' in row and ('denial' not in row and  'denied' not in row):
                    outcomes.append("Continued") 
            
     
             
            res['DecisionOutcomes'] = outcomes
            

            list_of_dict.append(res.copy())
            res = {}
        result = [list_of_dict,grands]
