...
def active_get(data):
    ...
    while True:
        ...
        if  len(lst[1].split()) == 1 and '/' in lst[1] and '/' in lst[2]:
        ...
        if  len(lst[1].split()) > 1:
            ...
            if (len(s) == 4 and  '/' in s[0] and '/' in s[1]) and ('/' in lst[2] or 'LEGACY CODES' in lst[2]):
                ...
                activeDutydict['RAD_Date'] = s[1]
                activeDutydict['Branch_Service']=s[2]
                activeDutydict['Discharge_Type']=s[3]
                del lst[1]
...
#P3OV10DP  ===> handwritten file,need more pdf files
#55J1A2IY  ===> pdf scanned  file,exported only 'noCompesation' Area and rating
